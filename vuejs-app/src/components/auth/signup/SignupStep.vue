      ...
      <div class="row justify-content-center">
        <div class="col-4">
          <button class="btn btn-primary btn-micro btn-block rounded-0">
            {{ 'forms.buttons.facebook' | translate }}
          </button>
        </div>

        <div class="col-4">
          <button class="btn btn-primary btn-micro btn-block rounded-0">
            {{ 'forms.buttons.googlePlus' | translate }}
          </button>
        </div>
      </div>
    </div>
    ...
    methods: {
      async doSignup () {
        ...
        try {
          ...
          const registeredUser = await this.$store.dispatch('doSignup', userDetails).then(response => response.data.user)
          this.signupStepIsDone(registeredUser)
        } catch (error) {
      ...
      signupStepIsDone (registeredUser) {
        this.$emit('signupStepDone', registeredUser)
      },
